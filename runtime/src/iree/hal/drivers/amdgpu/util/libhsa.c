...
// TODO(benvanik): move someplace central - status and stringbuilder have a
...
static bool iree_hal_amdgpu_libhsa_try_load_library_from_path(
  ...
  *out_library = NULL;
  ...
  // System APIs need cstrings and we will perform some path manipulation so we
  // do that locally in a heap-allocated string builder.
...
static iree_status_t iree_hal_amdgpu_libhsa_load_library(
  ...
  // This allows a hosting application to handle overrides in their own way.
  ...
  if (!library) {
    status =
        iree_make_status(IREE_STATUS_NOT_FOUND,
                         "HSA runtime library not available; ensure it is "
  ...
  if (iree_status_is_ok(status)) {
    ...
    if (!iree_status_is_ok(status) && out_libhsa->hsa_init) {
      ...
      iree_status_ignore(iree_dynamic_library_append_symbol_path_to_builder(
...
IREE_API_EXPORT iree_status_t iree_hal_amdgpu_libhsa_initialize(
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hsa_init(out_libhsa);
    if (iree_status_is_ok(status)) {
  ...
  // even when statically linking against libhsa.
  if (iree_status_is_ok(status)) {
    status = iree_status_annotate(
        iree_hsa_system_get_major_extension_table(
            out_libhsa, HSA_EXTENSION_AMD_LOADER, 1,
...
IREE_API_EXPORT void iree_hal_amdgpu_libhsa_deinitialize(
  ...
  if (libhsa->initialized) {
    IREE_IGNORE_ERROR(iree_hsa_shut_down(libhsa));
  }
...
iree_hal_amdgpu_libhsa_copy(const iree_hal_amdgpu_libhsa_t* libhsa,
  ...
  // pass it through out of an abundance of caution.
  IREE_RETURN_AND_END_ZONE_IF_ERROR(z0, iree_hsa_init(libhsa));
...
// names for free.
//
// If we wanted to enable statically link HSA we could use this mechanism to
// re-route all the iree_hsa_* methods directly to the HSA functions.
#define IREE_HAL_AMDGPU_LIBHSA_PFN_hsa_status_t(trace_category, result_type, \
                                                symbol, decl, args)          \
  iree_status_t iree_##symbol(const iree_hal_amdgpu_libhsa_t* IREE_RESTRICT  \
                                  libhsa _COMMA_DECL(decl)) {                \
    ...
    if (IREE_UNLIKELY(hsa_status != HSA_STATUS_SUCCESS &&                    \
      ...
      iree_status =                                                          \
          iree_make_status(iree_hsa_status_code(hsa_status), "[%s] %s: %s",  \
...
#define IREE_HAL_AMDGPU_LIBHSA_PFN_primitive(trace_category, result_type,    \
                                             symbol, decl, args)             \
  result_type iree_##symbol(const iree_hal_amdgpu_libhsa_t* IREE_RESTRICT    \
                                libhsa _COMMA_DECL(decl)) {                  \
    IREE_HAL_AMDGPU_LIBHSA_TRACE_ZONE_BEGIN_##trace_category(z0);            \
                                                                             \
