...
static iree_status_t iree_hal_amdgpu_compare_gpu_agents(
  ...
  *out_are_compatible = false;
  ...
  IREE_RETURN_IF_ERROR(iree_hsa_agent_get_info(
      libhsa, agent_a, HSA_AGENT_INFO_VENDOR_NAME, &vendor_name_a));
  char vendor_name_b[64] = {0};
  IREE_RETURN_IF_ERROR(iree_hsa_agent_get_info(
      libhsa, agent_b, HSA_AGENT_INFO_VENDOR_NAME, &vendor_name_b));
  ...
  // different revisions as this is just a human-friendly name. check same
  // marketing name HSA_AGENT_INFO_NAME
  char name_a[64] = {0};
  IREE_RETURN_IF_ERROR(
      iree_hsa_agent_get_info(libhsa, agent_a, HSA_AGENT_INFO_NAME, &name_a));
  char name_b[64] = {0};
  IREE_RETURN_IF_ERROR(
      iree_hsa_agent_get_info(libhsa, agent_b, HSA_AGENT_INFO_NAME, &name_b));
  ...
  IREE_RETURN_IF_ERROR(iree_hsa_agent_get_info(
      libhsa, agent_a, (hsa_agent_info_t)HSA_AMD_AGENT_INFO_CHIP_ID,
      &chip_id_a));
  uint32_t chip_id_b = 0;
  IREE_RETURN_IF_ERROR(iree_hsa_agent_get_info(
      libhsa, agent_b, (hsa_agent_info_t)HSA_AMD_AGENT_INFO_CHIP_ID,
      &chip_id_b));
...
iree_hal_amdgpu_topology_query_agent_compatibility(
  ...
  *out_is_compatible = false;
  ...
  hsa_device_type_t device_type = 0;
  IREE_RETURN_IF_ERROR(iree_hsa_agent_get_info(
      libhsa, agent, HSA_AGENT_INFO_DEVICE, &device_type));
  ...
  switch (device_type) {
    case HSA_DEVICE_TYPE_CPU: {
      if (topology->cpu_agent_count == 0) {
        *out_is_compatible = true;  // first agent
        return iree_ok_status();
      }
...
IREE_API_EXPORT iree_status_t iree_hal_amdgpu_topology_initialize_with_defaults(
  ...
  for (iree_host_size_t i = 0; i < agents.gpu_agent_count; ++i) {
    ...
    // Add the GPU agent and ensure its host CPU is also in the topology.
...
IREE_API_EXPORT iree_status_t
iree_hal_amdgpu_topology_initialize_from_device_mask(
    const iree_hal_amdgpu_libhsa_t* libhsa, uint64_t device_mask,
    iree_hal_amdgpu_topology_t* out_topology) {
  IREE_ASSERT_ARGUMENT(out_topology);
  IREE_TRACE_ZONE_BEGIN(z0);
  IREE_TRACE_ZONE_APPEND_VALUE_I64(z0, device_mask);
  ...
  iree_status_t status = iree_ok_status();
  for (iree_host_size_t device_ordinal = 0; device_ordinal < 64;
       ++device_ordinal) {
    if ((device_mask & (1ull << device_ordinal)) == 0) continue;
    status = iree_hal_amdgpu_topology_insert_gpu_agent_with_nearest_cpu_agent(
        out_topology, libhsa, agents.gpu_agents[device_ordinal]);
    if (!iree_status_is_ok(status)) break;
  }
