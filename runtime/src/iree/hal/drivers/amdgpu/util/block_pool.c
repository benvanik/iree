...
static iree_status_t iree_hal_amdgpu_block_pool_grow(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      ...
      iree_hsa_amd_memory_pool_allocate(
          block_pool->libhsa, block_pool->memory_pool,
  ...
  if (iree_status_is_ok(status)) {
    ...
    IREE_IGNORE_ERROR(
        iree_hsa_amd_memory_pool_free(block_pool->libhsa, base_ptr));
  }
...
void iree_hal_amdgpu_block_pool_trim(iree_hal_amdgpu_block_pool_t* block_pool) {
  ...
  // from it. Note that we already cleaned up the free block list above.
  ...
  while (allocation) {
    ...
    if (allocation->used_count == 0) {
      ...
      IREE_IGNORE_ERROR(iree_hsa_amd_memory_pool_free(block_pool->libhsa,
                                                      allocation->base_ptr));
...
static iree_status_t iree_hal_amdgpu_block_allocator_allocate_with_lock(
  ...
  while (block) {
    ...
    *out_token = (iree_hal_amdgpu_block_token_t){
