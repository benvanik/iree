...
iree_status_t iree_hal_amdgpu_vmem_ringbuffer_initialize(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_amd_vmem_address_reserve_align(
              libhsa, &out_ringbuffer->va_base_ptr, capacity * 3,
  ...
  iree_status_t status = iree_hsa_amd_vmem_handle_create(
      libhsa, memory_pool, capacity, MEMORY_TYPE_NONE,
  ...
  for (iree_host_size_t i = 0; iree_status_is_ok(status) && i < 3; ++i) {
    status = iree_hsa_amd_vmem_map(libhsa, va_offsets[i], capacity, 0,
  ...
  for (iree_host_size_t i = 0; iree_status_is_ok(status) && i < 3; ++i) {
    status = iree_hsa_amd_vmem_set_access(libhsa, va_offsets[i], capacity,
...
void iree_hal_amdgpu_vmem_ringbuffer_deinitialize(
  ...
  if (ringbuffer->alloc_handle.handle) {
    ...
    for (iree_host_size_t i = 0; i < IREE_ARRAYSIZE(va_offsets); ++i) {
      IREE_IGNORE_ERROR(
          iree_hsa_amd_vmem_unmap(libhsa, va_offsets[i], ringbuffer->capacity));
    }
    IREE_IGNORE_ERROR(
        iree_hsa_amd_vmem_handle_release(libhsa, ringbuffer->alloc_handle));
  }

  if (ringbuffer->va_base_ptr) {
    IREE_IGNORE_ERROR(iree_hsa_amd_vmem_address_free(
        libhsa, ringbuffer->va_base_ptr, ringbuffer->capacity * 3));
  }
