...
static iree_status_t iree_hal_amdgpu_agent_available_isas_append_to_builder(
  ...
  for (iree_host_size_t i = 0; i < isas->count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(iree_hsa_isa_get_info_alt(
        libhsa, isas->values[i], HSA_ISA_INFO_NAME_LENGTH, &isa_name_length));
    char* isa_name = NULL;
    IREE_RETURN_IF_ERROR(
        iree_string_builder_append_inline(builder, isa_name_length, &isa_name));
    IREE_RETURN_IF_ERROR(iree_hsa_isa_get_info_alt(
        libhsa, isas->values[i], HSA_ISA_INFO_NAME, isa_name));
  }
  return iree_ok_status();
}
...
static iree_status_t iree_hal_amdgpu_device_library_select_file(
  ...
  *out_file_toc = NULL;
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0,
      iree_hsa_agent_iterate_isas(
          libhsa, agent, iree_hal_amdgpu_iterate_agent_isa, &available_isas));
  ...
  for (iree_host_size_t i = 0; i < available_isas.count && !best_file_toc;
    ...
    uint32_t isa_name_length = 0;
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_hsa_isa_get_info_alt(libhsa, isa, HSA_ISA_INFO_NAME_LENGTH,
                                      &isa_name_length));
    char* isa_name_buffer = iree_alloca(isa_name_length);
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_hsa_isa_get_info_alt(libhsa, isa, HSA_ISA_INFO_NAME,
                                      isa_name_buffer));
...
iree_status_t iree_hal_amdgpu_device_library_initialize(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_code_object_reader_create_from_memory(
              libhsa, file_toc->data, file_toc->size, &code_object_reader));
  ...
  iree_status_t status = iree_hsa_executable_create_alt(
      libhsa, HSA_PROFILE_FULL, HSA_DEFAULT_FLOAT_ROUNDING_MODE_DEFAULT,
  ...
  if (iree_status_is_ok(status)) {
    for (iree_host_size_t i = 0; i < topology->gpu_agent_count; ++i) {
      status = iree_hsa_executable_load_agent_code_object(
          libhsa, out_library->executable, topology->gpu_agents[i],
  ...
  if (iree_status_is_ok(status)) {
    status =
        iree_hsa_executable_freeze(libhsa, out_library->executable, options);
  }

  // Release the reader now that the executable has been fully loaded.
  IREE_IGNORE_ERROR(
      iree_hsa_code_object_reader_destroy(libhsa, code_object_reader));
...
void iree_hal_amdgpu_device_library_deinitialize(
  ...
  if (library->executable.handle) {
    IREE_IGNORE_ERROR(
        iree_hsa_executable_destroy(library->libhsa, library->executable));
  }
...
static iree_status_t iree_hal_amdgpu_device_library_populate_kernel_args(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      ...
      iree_hsa_executable_get_symbol_by_name(
          libhsa, library->executable, symbol_name, &device_agent, &symbol),
  ...
  // somehow query? For now we hardcode.
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_executable_symbol_get_info(
              libhsa, symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT,
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      ...
      iree_hsa_executable_symbol_get_info(
          libhsa, symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_GROUP_SEGMENT_SIZE,
  ...
  // TODO(benvanik): intern an export_loc?
