...
static bool iree_hal_amdgpu_device_library_append_unique_arch_candidate(
    iree_string_view_t arch, iree_string_view_t* candidates,
    iree_host_size_t* inout_candidate_count) {
  if (iree_string_view_is_empty(arch)) return false;
...
static const iree_file_toc_t* iree_hal_amdgpu_device_library_find_file_for_isa(
    iree_string_view_t isa_name) {
  const iree_string_view_t isa_prefix = IREE_SVL("amdgcn-amd-amdhsa--");
  if (!iree_string_view_starts_with(isa_name, isa_prefix)) return NULL;
  iree_string_view_t exact_arch =
  ...
  iree_hal_amdgpu_device_library_append_unique_arch_candidate(
      exact_arch, arch_candidates, &arch_candidate_count);
  iree_hal_amdgpu_device_library_append_unique_arch_candidate(
      generic_arch, arch_candidates, &arch_candidate_count);

  iree_string_view_t exact_code_object_arch =
      iree_hal_amdgpu_device_library_code_object_arch_for_exact_arch(
          exact_arch);
  iree_hal_amdgpu_device_library_append_unique_arch_candidate(
      exact_code_object_arch, arch_candidates, &arch_candidate_count);
  iree_string_view_t generic_code_object_arch =
      iree_hal_amdgpu_device_library_code_object_arch_for_exact_arch(
          generic_arch);
  iree_hal_amdgpu_device_library_append_unique_arch_candidate(
      generic_code_object_arch, arch_candidates, &arch_candidate_count);

  for (iree_host_size_t i = 0; i < arch_candidate_count; ++i) {
    const iree_file_toc_t* file_toc =
        iree_hal_amdgpu_device_library_find_file_for_arch(arch_candidates[i]);
    if (file_toc) return file_toc;
  }
  return NULL;
}
...
static iree_status_t iree_hal_amdgpu_device_library_select_file(
  ...
  *out_file_toc = NULL;
  ...
  for (iree_host_size_t i = 0; i < available_isas.count && !best_file_toc;
    ...
    best_file_toc = iree_hal_amdgpu_device_library_find_file_for_isa(isa_name);
    if (best_file_toc) {
      best_isa = isa;
      break;
    }
