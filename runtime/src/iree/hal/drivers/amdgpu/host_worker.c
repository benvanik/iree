...
static iree_status_t iree_hal_amdgpu_host_worker_issue_barrier_and(
  ...
  for (iree_host_size_t i = 0; i < IREE_ARRAYSIZE(packet->dep_signal); ++i) {
    ...
    iree_hsa_signal_wait_scacquire(libhsa, packet->dep_signal[i],
                                   HSA_SIGNAL_CONDITION_EQ, 0u, UINT64_MAX,
                                   HSA_WAIT_STATE_BLOCKED);
  }

  if (packet->completion_signal.handle != 0) {
    iree_hsa_signal_subtract_screlease(libhsa, packet->completion_signal, 1);
  }
...
static iree_status_t iree_hal_amdgpu_host_worker_issue_barrier_or(
  ...
  const uint32_t satisfying_index = iree_hsa_amd_signal_wait_any(
      libhsa, signal_count, signals, conds, values, UINT64_MAX,
      HSA_WAIT_STATE_BLOCKED, &satisfying_value);
  if (satisfying_index != UINT32_MAX) {
    iree_hsa_signal_load_scacquire(libhsa, signals[satisfying_index]);
  }
...
static iree_status_t iree_hal_amdgpu_host_worker_issue_barrier_value(
  ...
  if (packet->mask == UINT64_MAX) {
    ...
    iree_hsa_signal_wait_scacquire(libhsa, packet->signal, packet->cond,
                                   packet->value, UINT64_MAX,
                                   HSA_WAIT_STATE_BLOCKED);
  } else {
    ...
    hsa_signal_value_t value =
        iree_hsa_signal_load_scacquire(libhsa, packet->signal);
    while (!iree_hsa_condition_is_met(packet->cond, value & packet->mask,
                                      packet->value)) {
      value = iree_hsa_signal_wait_scacquire(
          libhsa, packet->signal, HSA_SIGNAL_CONDITION_NE, value, UINT64_MAX,
          HSA_WAIT_STATE_BLOCKED);
    }
  }
...
static iree_status_t iree_hal_amdgpu_host_worker_barrier(
  ...
  hsa_signal_value_t value = iree_hsa_signal_wait_scacquire(
      libhsa, worker->outstanding_signal, HSA_SIGNAL_CONDITION_LT, 1ull,
...
iree_status_t iree_hal_amdgpu_host_worker_initialize(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_agent_get_info(&system->libhsa, host_agent,
  ...
  iree_status_t status = iree_hsa_amd_signal_create(
      &system->libhsa, 0ull, 1, &host_agent,
  ...
  if (iree_status_is_ok(status)) {
    status =
        iree_hsa_amd_signal_create(&system->libhsa, 0ull, 1, &host_agent,
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hsa_soft_queue_create(
        &system->libhsa, system->host_memory_pools[host_ordinal].fine_region,
...
void iree_hal_amdgpu_host_worker_deinitialize(
  ...
  if (worker->queue) {
    IREE_IGNORE_ERROR(iree_hsa_queue_inactivate(libhsa, worker->queue));
  }

  // Signal the doorbell to the termination value. This should wake the worker
  // and have it exit.
  if (worker->doorbell.handle) {
    iree_hsa_signal_store_screlease(libhsa, worker->doorbell, UINT64_MAX);
  }
  ...
  if (worker->queue) {
    IREE_IGNORE_ERROR(iree_hsa_queue_destroy(libhsa, worker->queue));
  }
  if (worker->doorbell.handle) {
    IREE_IGNORE_ERROR(iree_hsa_signal_destroy(libhsa, worker->doorbell));
  }
  if (worker->outstanding_signal.handle) {
    IREE_IGNORE_ERROR(
        iree_hsa_signal_destroy(libhsa, worker->outstanding_signal));
  }
...
void iree_hal_amdgpu_host_worker_notify_completion(
  ...
  iree_hsa_signal_subtract_screlease(&worker->system->libhsa,
