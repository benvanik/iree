...
#define IREE_HAL_AMDGPU_TRACING_FEATURE_DEVICE_EXECUTION (1 << 1)

// Tracks all device allocations.
#define IREE_HAL_AMDGPU_TRACING_FEATURE_ALLOCATION_TRACKING (1 << 2)

// Forwards log messages to traces, which will be visible under "Messages" in
// the Tracy UI.
#define IREE_HAL_AMDGPU_TRACING_FEATURE_LOG_MESSAGES (1 << 3)
...
typedef struct IREE_OCL_ALIGNAS(8)
  ...
  // assigned on the host when the execution context is configured.
  iree_hal_amdgpu_trace_executor_id_t executor_id;
  uint8_t reserved[1];  // may be uninitialized
  // A query ID used to feed back the timestamp once the execution has
  // completed. For combined leaf zones this is two consecutive IDs for the
  // begin and end respectively.
  iree_hal_amdgpu_trace_execution_query_id_t execution_query_ids;
  // A reference into the interned export source location table.
  ...
  // device.
  uint32_t export_loc;
  // Event ordinal within the parent execution zone.
  // As issuing happens in parallel we need a way to take the out-of-order zones
  // and provide them to the trace sink as an ordered set. We do this by
  // taking the timestamp of the parent zone and adding the ordinal. This lets
  // us at least see the sequence and correlate the execution to the issue even
  // if the timestamps aren't very useful.
  uint32_t issue_ordinal;
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_plot_config_s {
  ...
  // 0xAARRGGBB color of the plot. Alpha should be 0xFF, e.g. 0xFF000000.
  uint32_t color;
...
// Begins a new zone with the given compile-time |name_literal| and XRGB color.
// The literal must be static const and will be embedded in the trace buffer by
// reference.
