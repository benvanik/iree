...
iree_status_t iree_hal_amdgpu_queue_options_verify(
  ...
  IREE_RETURN_IF_ERROR(iree_hsa_agent_get_info(
      libhsa, agent, HSA_AGENT_INFO_QUEUE_MIN_SIZE, &queue_min_size));
  uint32_t queue_max_size = 0;
  IREE_RETURN_IF_ERROR(iree_hsa_agent_get_info(
      libhsa, agent, HSA_AGENT_INFO_QUEUE_MAX_SIZE, &queue_max_size));
...
void iree_hal_amdgpu_kernel_dispatch(
  ...
  if (completion_signal.handle) {
    iree_hsa_signal_add_relaxed(libhsa, completion_signal, 1u);
  }
...
static iree_status_t iree_hal_amdgpu_create_control_queue(
  ...
  *out_queue = NULL;
  ...
  iree_status_t status = iree_hsa_queue_create(
      libhsa, device_agent, queue_capacity, HSA_QUEUE_TYPE_MULTI,
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hsa_amd_queue_set_priority(libhsa, queue,
  ...
  if (iree_status_is_ok(status)) {
    *out_queue = queue;
    ...
    if (queue) {
      IREE_IGNORE_ERROR(iree_hsa_queue_destroy(libhsa, queue));
    }
  }
  ...
  *out_queue = NULL;
  ...
  if (iree_status_is_ok(status)) {
    ...
    status = iree_hsa_amd_profiling_set_profiler_enabled(
        libhsa, queue, profiler_enabled ? 1 : 0);
  }
...
static iree_status_t iree_hal_amdgpu_allocate_scheduler(
  ...
  *out_scheduler = NULL;
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_amd_memory_pool_allocate(libhsa, memory_pool, total_size,
  ...
  iree_status_t status = iree_hsa_amd_agents_allow_access(
      libhsa, IREE_ARRAYSIZE(access_agents), access_agents, NULL, slab);
  ...
  if (iree_status_is_ok(status)) {
    ...
    *out_scheduler = scheduler;
    ...
    IREE_IGNORE_ERROR(iree_hsa_amd_memory_pool_free(libhsa, slab));
  }
  IREE_TRACE_ZONE_END(z0);
  return status;
}
...
static void iree_hal_amdgpu_free_scheduler(
  ...
  IREE_IGNORE_ERROR(iree_hsa_amd_memory_pool_free(libhsa, scheduler));
...
iree_status_t iree_hal_amdgpu_queue_initialize(
  ...
  // Start idle (0).
  if (iree_status_is_ok(status)) {
    status = iree_hsa_amd_signal_create(libhsa, 0ull, 0, NULL, 0,
...
void iree_hal_amdgpu_queue_deinitialize(iree_hal_amdgpu_queue_t* queue) {
  ...
  if (queue->idle_signal.handle) {
    iree_hsa_signal_wait_scacquire(libhsa, queue->idle_signal,
  ...
  if (queue->idle_signal.handle) {
    IREE_IGNORE_ERROR(iree_hsa_signal_destroy(libhsa, queue->idle_signal));
  }

  for (iree_host_size_t i = 0; i < queue->execution_queue_count; ++i) {
    hsa_queue_t* execution_queue = queue->execution_queues[i];
    if (execution_queue) {
      IREE_IGNORE_ERROR(iree_hsa_queue_destroy(libhsa, execution_queue));
    }
  }
  if (queue->control_queue) {
    IREE_IGNORE_ERROR(iree_hsa_queue_destroy(libhsa, queue->control_queue));
  }
