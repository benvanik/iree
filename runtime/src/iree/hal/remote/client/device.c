...
static iree_status_t iree_hal_remote_client_device_wait_semaphores(
  ...
  // Semaphore waits are handled locally using the frontier system.
  // The client tracks semaphore values and receives updates from the server
  // as operations complete. No network round-trip needed for waits if the
  // local state is already up-to-date.
  (void)device;
  return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                          "remote wait_semaphores not yet implemented");
}
...
static const iree_hal_device_vtable_t iree_hal_remote_client_device_vtable = {
    ...
    .wait_semaphores = iree_hal_remote_client_device_wait_semaphores,
