...
//   timer.base.opcode = IREE_ASYNC_OPCODE_TIMER;
//   timer.base.callback = on_timeout;
...
// Forward declaration - full definition in iree/async/event.h.
...
//   iree_async_sequence_operation_t* seq = slab_alloc(
//       iree_async_sequence_operation_size(3));
...
//   seq->base.callback = on_pipeline_complete;
...
// Returns the total allocation size needed for a sequence with |step_count|
// steps. Includes the struct and trailing step pointer storage.
static inline iree_host_size_t iree_async_sequence_operation_size(
    iree_host_size_t step_count) {
  return sizeof(iree_async_sequence_operation_t) +
         step_count * sizeof(iree_async_operation_t*);
}
...
// Forward declaration - full definition in platform implementations.
