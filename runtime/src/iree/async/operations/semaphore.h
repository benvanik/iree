...
//   wait.base.opcode = IREE_ASYNC_OPCODE_SEMAPHORE_WAIT;
//   wait.base.callback = on_gpu_complete;
...
// Returns the total allocation size for a wait operation with |count|
// semaphores. Includes the struct and trailing semaphore + value arrays.
static inline iree_host_size_t iree_async_semaphore_wait_operation_size(
    iree_host_size_t count) {
  return sizeof(iree_async_semaphore_wait_operation_t) +
         count * sizeof(iree_async_semaphore_t*) + count * sizeof(uint64_t);
}
...
// Returns the total allocation size for a signal operation with |count|
// semaphores. Includes the struct and trailing semaphore + value arrays.
static inline iree_host_size_t iree_async_semaphore_signal_operation_size(
    iree_host_size_t count) {
  return sizeof(iree_async_semaphore_signal_operation_t) +
         count * sizeof(iree_async_semaphore_t*) + count * sizeof(uint64_t);
}
