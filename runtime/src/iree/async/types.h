...
// inline helpers and forward declarations are included here; no function
...
// Cleans up all registrations. Call from buffer destroy, before freeing the
// buffer's memory. Walks the list and invokes each entry's cleanup_fn.
static inline void iree_async_buffer_registration_state_cleanup(
    iree_async_buffer_registration_state_t* state) {
