...
iree_async_posix_ready_op_t* iree_async_posix_ready_pool_acquire(
  ...
  ready_op->operation = NULL;
  ready_op->poll_events = IREE_ASYNC_POLL_EVENT_NONE;

  return ready_op;
}

void iree_async_posix_ready_pool_release(
    iree_async_posix_ready_pool_t* pool,
    iree_async_posix_ready_op_t* ready_op) {
  ready_op->operation = NULL;
  ready_op->poll_events = IREE_ASYNC_POLL_EVENT_NONE;

  iree_atomic_slist_push(&pool->free_list, &ready_op->slist_entry);
}
