...
typedef struct iree_async_proactor_io_uring_t {
  ...
  // Number of buffers currently registered in the fixed buffer table.
  // io_uring allows only one buffer table at a time (IORING_REGISTER_BUFFERS).
  // We track this to:
  //   - Know when to use IORING_UNREGISTER_BUFFERS on cleanup
  //   - Return appropriate errors if multiple registrations are attempted
  uint16_t registered_buffer_count;
...
typedef struct iree_async_io_uring_fence_import_tracker_t {
  iree_async_semaphore_t* semaphore;  // Retained; released after signal/fail.
  uint64_t signal_value;              // Value to signal semaphore to.
  int fence_fd;                       // Owned fd; closed on completion.
...
typedef struct iree_async_io_uring_fence_export_tracker_t {
  iree_async_semaphore_timepoint_t
      timepoint;  // Embedded; owned by semaphore until callback.
  iree_async_semaphore_t* semaphore;  // Retained; released in callback.
  int eventfd;  // Written to on signal (caller-owned, not closed).
  iree_allocator_t allocator;  // For freeing this tracker.
...
// Called on submit rollback (from proactor_submit.c) and after completion
// callbacks (from proactor.c).
