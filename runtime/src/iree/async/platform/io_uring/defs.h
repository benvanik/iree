...
//===----------------------------------------------------------------------===//
// Futex flags (for FUTEX_WAIT/WAKE/WAITV)
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// MSG_RING command types and flags (for IORING_OP_MSG_RING)
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// Poll event bits (for IORING_OP_POLL_ADD)
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// Socket message flags (for sqe->msg_flags in SEND/SENDMSG/RECV/RECVMSG)
//===----------------------------------------------------------------------===//
...
typedef struct iree_io_uring_sqe {
  ...
  uint16_t ioprio;  // I/O priority or request-specific flags.
  int32_t fd;       // File descriptor (or fixed file index).
  ...
  union {
    ...
    struct {
      uint32_t level;    // Socket option level (for SOCKET cmd).
      uint32_t optname;  // Socket option name (for SOCKET cmd).
    };
  };
  ...
  union {
    ...
    uint16_t poll_events;       // Poll events (16 bits used).
  ...
  union {
    ...
    uint32_t file_index;   // Fixed file index for direct ops.
    uint32_t optlen;       // Socket option length (for SOCKET cmd).
    struct {
      uint16_t addr_len;
      uint16_t __pad3[1];
    };
  ...
  union {
    ...
    uint64_t optval;  // Socket option value pointer (for SOCKET cmd).
    uint8_t cmd[0];   // Uring command data (variable length).
  };
} iree_io_uring_sqe_t;
...
typedef struct iree_io_uring_cqe {
  uint64_t user_data;  // Copied from SQE for correlation.
  int32_t res;         // Result: bytes transferred or -errno.
...
//===----------------------------------------------------------------------===//
// Provided buffer ring structures (kernel 5.19+)
//===----------------------------------------------------------------------===//
...
typedef struct iree_io_uring_buf_reg {
  ...
  uint32_t ring_entries;  // Number of entries (power of 2, max 32768).
...
typedef struct iree_io_uring_buf {
  ...
  uint16_t bid;   // Buffer ID (returned in CQE).
  uint16_t resv;
} iree_io_uring_buf_t;
...
//===----------------------------------------------------------------------===//
// Atomic helpers for kernel-shared memory
//===----------------------------------------------------------------------===//
