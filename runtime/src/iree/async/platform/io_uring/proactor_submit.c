...
static void iree_async_proactor_io_uring_fill_event_wait(
  ...
  // Retain once for the operation (released via release_operation_resources
  // when the READ CQE is processed).
...
static void iree_async_proactor_io_uring_fill_socket_connect(
  ...
  iree_async_socket_retain(connect->socket);
...
// Retains the listening socket to ensure it outlives the in-flight operation.
static void iree_async_proactor_io_uring_fill_socket_accept(
  ...
  // Retain the listening socket to ensure it outlives the in-flight operation.
...
static iree_status_t iree_async_proactor_io_uring_fill_socket_recv(
  ...
  // Validate buffer count before retaining any resources.
...
static iree_status_t iree_async_proactor_io_uring_fill_socket_send(
  ...
  if (send->buffers.count == 1) {
    ...
    // Check if we can use fixed-buffer path for true zero-copy.
    // Fixed buffers avoid per-operation page pinning overhead.
    // Only check eligibility when actually using ZC.
    ...
    if (use_zero_copy) {
      IREE_RETURN_IF_ERROR(iree_async_span_check_fixed_buffer_send(
          proactor, first_buffer, &eligibility, &fixed_buffer_index));
    }
...
static iree_status_t iree_async_proactor_io_uring_fill_socket_sendto(
  ...
  // Retain the socket to ensure it outlives the in-flight operation.
  iree_async_socket_retain(sendto->socket);
...
static void iree_async_proactor_io_uring_fill_notification_signal_futex(
  ...
  iree_async_notification_retain(signal->notification);
...
// Returns the allocation size for a semaphore wait tracker with |count|
// semaphores.
static inline iree_host_size_t iree_async_io_uring_semaphore_wait_tracker_size(
    iree_host_size_t count) {
  return sizeof(iree_async_io_uring_semaphore_wait_tracker_t) +
         count * sizeof(iree_async_semaphore_timepoint_t);
}

// Forward declaration for the timepoint callback.
...
static iree_status_t iree_async_proactor_io_uring_submit_semaphore_wait(
  ...
  iree_host_size_t tracker_size =
      iree_async_io_uring_semaphore_wait_tracker_size(wait_op->count);
...
// Releases resources retained during SQE preparation for a single operation.
// Called on submit rollback and after completion callbacks.
void iree_async_proactor_io_uring_release_operation_resources(
  ...
  switch (operation->type) {
    ...
    case IREE_ASYNC_OPERATION_TYPE_SOCKET_CONNECT: {
      ...
      iree_async_socket_release(connect->socket);
      break;
    }
    ...
    case IREE_ASYNC_OPERATION_TYPE_SOCKET_SENDTO: {
      ...
      iree_async_socket_release(sendto->socket);
      break;
    }
    ...
    case IREE_ASYNC_OPERATION_TYPE_SOCKET_CLOSE: {
      // The close operation consumes the caller's reference.
      // After the close completes, we release that reference, destroying the
      // socket (since the fd was already closed by the kernel).
      iree_async_socket_close_operation_t* close_op =
    ...
    case IREE_ASYNC_OPERATION_TYPE_NOTIFICATION_WAIT: {
      ...
      iree_async_notification_release(wait->notification);
      break;
    }
...
// Releases resources retained during SQE preparation for a batch of operations.
...
static void iree_async_proactor_io_uring_release_prepared(
  ...
  for (iree_host_size_t i = 0; i < prepared_count; ++i) {
    ...
    iree_async_proactor_io_uring_release_operation_resources(operation);
  }
}
  ...
  for (iree_host_size_t i = 0; i < effective_count; ++i) {
    ...
    if (needs_two_sqes) {
      ...
      if (!poll_sqe || !read_sqe) {
        ...
        iree_io_uring_ring_sq_unlock(&proactor->ring);
        iree_async_proactor_io_uring_release_prepared(operations, i);
        return iree_make_status(IREE_STATUS_RESOURCE_EXHAUSTED,
    ...
    if (!iree_status_is_ok(status)) {
      // Rollback all SQEs and release retained resources for operations that
      // were successfully prepared. The current operation (i) failed and did
      // not retain resources, so only release operations [0, i).
