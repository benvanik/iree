...
// Frees an event set allocated with one of the create functions.
...
//
// Returns IREE_STATUS_DEADLINE_EXCEEDED if the timeout expires with no events.
// Returns IREE_STATUS_OK if interrupted by a signal (with ready_count 0).
...
// Creates an event set using the specified |backend|.
// Returns IREE_STATUS_UNAVAILABLE if |backend| is not supported on this
// platform (e.g., EPOLL on macOS returns UNAVAILABLE).
iree_status_t iree_async_posix_event_set_create(
    iree_async_posix_event_backend_t backend, iree_allocator_t allocator,
    iree_async_posix_event_set_t** out_event_set);

// Creates an event set using poll(). Available on all POSIX platforms.
iree_status_t iree_async_posix_event_set_create_poll(
    iree_allocator_t allocator, iree_async_posix_event_set_t** out_event_set);

#if defined(IREE_PLATFORM_LINUX)
// Creates an event set using Linux epoll for O(1) event notification.
iree_status_t iree_async_posix_event_set_create_epoll(
    iree_allocator_t allocator, iree_async_posix_event_set_t** out_event_set);
#endif  // IREE_PLATFORM_LINUX

#if defined(IREE_PLATFORM_APPLE) || defined(IREE_PLATFORM_BSD)
// Creates an event set using BSD/macOS kqueue for O(1) event notification.
iree_status_t iree_async_posix_event_set_create_kqueue(
    iree_allocator_t allocator, iree_async_posix_event_set_t** out_event_set);
