...
static void iree_async_proactor_posix_release_operation_resources(
    iree_async_operation_t* operation);

static void iree_async_proactor_posix_signal_teardown(
    iree_async_proactor_posix_t* proactor);
...
iree_status_t iree_async_proactor_create_posix_with_backend(
  ...
  *out_proactor = NULL;
  ...
  // Futex-based sync notification waits are available — report
  // FUTEX_OPERATIONS so notification-source relays can use futex mode.
  supported_capabilities |= IREE_ASYNC_PROACTOR_CAPABILITY_FUTEX_OPERATIONS;
  ...
  if (iree_status_is_ok(status)) {
    status = iree_async_posix_event_set_create(event_backend, allocator,
                                               &proactor->event_set);
  }
  if (iree_status_is_ok(status)) {
...
static void iree_async_proactor_posix_destroy(
  ...
  if (proactor->signal.initialized) {
    iree_async_proactor_posix_signal_teardown(proactor);
  }
...
// Pushes a completion for immediate delivery in poll().
...
// Pushes an operation to the pending_queue for the poll thread to process.
// The pending_queue is an MPSC queue: any thread may push, only the poll thread
// pops. This ensures all fd_map, event_set, and timer_list mutations happen
// exclusively on the poll thread, avoiding data races.
//
...
static iree_status_t iree_async_proactor_posix_register_fd_operation(
  ...
  iree_status_t status = iree_async_posix_fd_map_insert_or_chain(
...
static void iree_async_proactor_posix_drain_pending_queue(
  ...
  while ((entry = iree_atomic_slist_pop(&proactor->pending_queue)) != NULL) {
    ...
    if (iree_any_bit_set(operation->internal_flags,
      ...
      // If this is an fd operation, cancel() incremented the fd cancellation
      // counter. Decrement it here since the operation was never registered
      // in the fd_map (it was still in the pending queue).
      ...
      if (!iree_status_is_ok(push_status)) {
        ...
        iree_async_proactor_posix_release_operation_resources(operation);
...
static iree_status_t iree_async_proactor_posix_submit_socket_accept(
  ...
  iree_async_socket_retain(accept_op->listen_socket);
  ...
  if (accepted_fd >= 0) {
    // Immediate success (pending connection was already queued by the kernel).
    iree_async_socket_t* accepted_socket = NULL;
    iree_status_t status = iree_allocator_malloc(proactor->base.allocator,
                                                 sizeof(*accepted_socket),
                                                 (void**)&accepted_socket);
    if (!iree_status_is_ok(status)) {
      close(accepted_fd);
      return iree_async_proactor_posix_complete_immediately(
          proactor, &accept_op->base, status, IREE_ASYNC_COMPLETION_FLAG_NONE);
    }
    ...
    if (iree_any_bit_set(accept_op->base.flags,
                         IREE_ASYNC_OPERATION_FLAG_MULTISHOT)) {
      flags = IREE_ASYNC_COMPLETION_FLAG_MORE;
    }
    return iree_async_proactor_posix_complete_immediately(
        proactor, &accept_op->base, iree_ok_status(), flags);
  }
  ...
  // Immediate failure (e.g., EINVAL for non-listening socket).
  return iree_async_proactor_posix_complete_immediately(
      proactor, &accept_op->base,
...
// and that the region was registered with this proactor. Retains the socket
// for the duration of the in-flight operation. The actual recv is deferred to
// the poll thread: the operation enters the pending queue and is executed when
// the socket fd becomes readable.
static iree_status_t iree_async_proactor_posix_submit_recv_pool(
    iree_async_proactor_posix_t* proactor,
    iree_async_socket_recv_pool_operation_t* recv_pool) {
  // Validate the buffer pool region before retaining any resources.
  ...
  // Retain the socket to ensure it outlives the in-flight operation.
...
// Submits an EVENT_WAIT by retaining the event and deferring to the poll
// thread. The poll thread releases the reference on completion or cancellation.
...
// Submits a NOTIFICATION_WAIT by retaining the notification, capturing the
// current epoch as the wait token, and deferring to the poll thread.
// The poll thread releases the reference on completion or cancellation.
static iree_status_t iree_async_proactor_posix_submit_notification_wait(
    iree_async_proactor_posix_t* proactor,
    iree_async_notification_wait_operation_t* wait) {
  iree_async_notification_retain(wait->notification);
  wait->wait_token =
...
// Forward declarations for execute functions called from the submit path.
...
static iree_status_t iree_async_proactor_posix_submit(
  ...
  for (iree_host_size_t i = 0; i < operations.count; ++i) {
    ...
    // Clear internal flags for fresh submission.
    ...
    switch (operation->type) {
      case IREE_ASYNC_OPERATION_TYPE_NOP:
        status = iree_async_proactor_posix_complete_immediately(
            proactor, operation, iree_ok_status(),
            IREE_ASYNC_COMPLETION_FLAG_NONE);
        break;

      case IREE_ASYNC_OPERATION_TYPE_SOCKET_ACCEPT:
        status = iree_async_proactor_posix_submit_socket_accept(
      ...
      case IREE_ASYNC_OPERATION_TYPE_SOCKET_RECV: {
        ...
        iree_async_socket_retain(recv_op->socket);
      ...
      case IREE_ASYNC_OPERATION_TYPE_SOCKET_SEND: {
        ...
        iree_async_socket_retain(send_op->socket);
        ...
        if (send_result == IREE_ASYNC_IO_WOULD_BLOCK) {
          // Cannot defer: data buffers may not outlive this call.
      ...
      case IREE_ASYNC_OPERATION_TYPE_SOCKET_RECVFROM: {
        ...
        iree_async_proactor_posix_materialize_iovecs(
            ...
            &recvfrom_op->buffers);
        iree_async_socket_retain(recvfrom_op->socket);
        iree_async_proactor_posix_push_pending(proactor, operation);
        break;
      }
      case IREE_ASYNC_OPERATION_TYPE_SOCKET_SENDTO: {
        ...
        iree_async_proactor_posix_materialize_iovecs(
            (struct iovec*)sendto_op->platform.posix.iovecs,
            &sendto_op->buffers);
...
static void iree_async_proactor_posix_submit_continuation_chain(
  ...
  iree_status_t submit_status =
      iree_async_proactor_posix_submit(&proactor->base, chain_list);
...
static iree_host_size_t iree_async_proactor_posix_process_expired_timers(
  ...
  while ((timer = iree_async_posix_timer_list_pop_expired(&proactor->timers,
    ...
    // set by an earlier callback in this loop via cancel().

    // 2. Determine status based on cancelled flag.
    iree_status_t status = iree_ok_status();
    ...
    // 3. Push to completion queue (consistent with socket ops).
    iree_async_posix_completion_t* completion =
        iree_async_posix_completion_pool_acquire(&proactor->completion_pool);
    if (completion) {
...
// Releases resources retained during submit for an operation that has completed
// its final completion (no MORE flag). Called from the completion drain path.
...
static void iree_async_proactor_posix_release_operation_resources(
  ...
  switch (operation->type) {
    ...
    case IREE_ASYNC_OPERATION_TYPE_SOCKET_CLOSE: {
      // The close operation consumes the caller's reference. After the close
      // completes, we release that reference, destroying the socket (since the
      // fd was already closed by the kernel).
      iree_async_socket_close_operation_t* close_op =
...
static void iree_async_proactor_posix_process_operation_chain(
  ...
  while (current != NULL) {
    ...
    if (iree_any_bit_set(current->internal_flags,
      ...
      if (!iree_status_is_ok(push_status)) {
        iree_status_ignore(push_status);
        iree_async_proactor_posix_release_operation_resources(current);
        iree_async_proactor_posix_dispatch_linked_continuation(
            proactor, current, iree_status_from_code(IREE_STATUS_CANCELLED));
    ...
    if (epoch_advanced || cancelled) {
      ...
      *previous = next;
      ...
      if (completion) {
        ...
        // Pool exhausted — release resources and dispatch directly.
        iree_async_proactor_posix_release_operation_resources(&wait->base);
        iree_async_proactor_posix_dispatch_linked_continuation(
  ...
  for (iree_host_size_t i = 0; i < map->bucket_count; ++i) {
    ...
    do {
      ...
      while (op) {
        ...
        if (!iree_status_is_ok(push_status)) {
          iree_status_ignore(push_status);
          iree_async_proactor_posix_release_operation_resources(op);
          iree_async_proactor_posix_dispatch_linked_continuation(
              proactor, op, iree_status_from_code(IREE_STATUS_CANCELLED));
          if (op->completion_fn) {
            op->completion_fn(op->user_data, op,
...
static iree_status_t iree_async_proactor_posix_poll(
  ...
  // Process any pending fd cancellations (cancel() called on fd-registered
  // operations whose fds may not fire). Pushes CANCELLED completions.
  ...
  iree_status_t poll_status = iree_async_posix_event_set_wait(
      proactor->event_set, timeout_ms, &ready_count);
  if (!iree_status_is_ok(poll_status)) {
    if (iree_status_code(poll_status) == IREE_STATUS_DEADLINE_EXCEEDED) {
      ...
      // Process pending fd cancellations (cancel() called during wait).
      iree_async_proactor_posix_drain_pending_fd_cancellations(proactor);
  ...
  // Process any pending fd cancellations not handled during ready-fd dispatch
  // (e.g., cancelled operations on fds that didn't fire this poll cycle).
  iree_async_proactor_posix_drain_pending_fd_cancellations(proactor);
...
static iree_status_t iree_async_proactor_posix_cancel(
  ...
  switch (operation->type) {
    case IREE_ASYNC_OPERATION_TYPE_TIMER: {
      // Set the cancelled flag. The poll thread checks this during timer
      // expiration processing and during pending_queue drain.
      // This is thread-safe: internal_flags is only read by the poll thread
      // after acquiring the operation from a queue or list.
...
#define iree_async_posix_signal_teardown iree_async_linux_signal_teardown
...
#define iree_async_posix_signal_teardown iree_async_selfpipe_signal_teardown
...
static iree_status_t iree_async_proactor_posix_signal_initialize(
  ...
  if (iree_status_is_ok(status)) {
    ...
    iree_async_posix_signal_teardown(&proactor->signal.backend_state);
  }
...
// Tears down all signal handling state. Called from proactor destroy.
static void iree_async_proactor_posix_signal_teardown(
    iree_async_proactor_posix_t* proactor) {
  iree_allocator_t allocator = proactor->base.allocator;
  ...
  // Tear down backend signal state (closes fd, restores sigmask/handlers).
  iree_async_posix_signal_teardown(&proactor->signal.backend_state);
