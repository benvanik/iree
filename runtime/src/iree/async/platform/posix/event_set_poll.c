...
iree_status_t iree_async_posix_event_set_create(
  ...
  switch (backend) {
    case IREE_ASYNC_POSIX_EVENT_BACKEND_DEFAULT:
      ...
      return iree_async_posix_event_set_create_epoll(allocator, out_event_set);
#elif defined(IREE_PLATFORM_APPLE) || defined(IREE_PLATFORM_BSD)
      // Use kqueue on BSD/macOS for O(1) performance.
      return iree_async_posix_event_set_create_kqueue(allocator, out_event_set);
#else
      // Fall through to poll on other platforms.
      return iree_async_posix_event_set_create_poll(allocator, out_event_set);
#endif
