...
static iree_status_t iree_async_posix_execute_file_close(
  ...
  // Mark the fd as invalid in the file structure to prevent double-close.
  op->file->primitive.value.fd = -1;

  // Release the file reference (this will trigger destroy if ref count is 0).
  iree_async_file_release(op->file);
  op->file = NULL;
...
void iree_async_posix_worker_request_exit(iree_async_posix_worker_t* worker) {
  ...
  // Wake the worker in case it's waiting for work.
