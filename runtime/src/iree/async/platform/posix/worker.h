...
//
// NOTE: Field layout is carefully ordered to minimize false sharing.
// Hot fields (touched by other threads) are grouped together, followed by
// padding, then cold fields (thread-local). When workers are stored in a
// contiguous array, this ensures atomic operations don't thrash neighbors.
typedef struct iree_async_posix_worker_t {
  // === Hot fields (touched by other threads) ===

  // Signaled when worker should wake from idle wait.
  // Other threads post to this when pushing work to ready_queue.
  ...
  // === Cold fields (thread-local, only touched by this worker) ===
  ...
  // Padding to ensure struct spans at least one cache line to minimize false
  // sharing when workers are stored in a contiguous array.
  uint8_t _padding[16];
