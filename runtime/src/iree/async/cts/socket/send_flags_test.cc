...
TEST_P(SendFlagsTest, ZeroCopySendRegisteredSlab) {
  ...
  IREE_ASSERT_OK(
      iree_async_slab_allocate(&slab_options, iree_allocator_system(), &slab));
  ...
  if (iree_status_is_failed_precondition(status)) {
    // Some backends (e.g., io_uring) only allow one READ region per proactor.
    // Skip if registration fails due to this constraint.
...
TEST_P(SendFlagsTest, ZeroCopySendPartialBuffer) {
  ...
  iree_async_span_t partial_span = {
      .region = lease.span.region,
      .offset = lease.span.offset + partial_offset,
...
TEST_P(SendFlagsTest, ConcurrentRegisteredSends) {
  // This test requires zerocopy capability because it uses registered buffers
  // with the ZERO_COPY socket option. Without zerocopy support, the send
  // operations may hang or timeout.
  if (!(capabilities_ & IREE_ASYNC_PROACTOR_CAPABILITY_ZERO_COPY_SEND)) {
    GTEST_SKIP() << "Backend lacks ZERO_COPY_SEND capability";
    return;
  }
...
TEST_P(SendFlagsTest, ZeroCopySendMultipleSlabs) {
  ...
  IREE_ASSERT_OK(iree_async_slab_allocate(&slab_a_options,
                                          iree_allocator_system(), &slab_a));
  ...
  IREE_ASSERT_OK(iree_async_slab_allocate(&slab_b_options,
                                          iree_allocator_system(), &slab_b));
