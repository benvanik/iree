...
//
// These tests verify multishot accept and recv operations where a single SQE
// produces multiple CQEs with CQE_F_MORE set on all but the final completion.
// Requires kernel 5.19+ for multishot support.
...
// These tests verify multishot operations where a single SQE produces multiple
// CQEs with CQE_F_MORE set on all but the final completion. The tests use
// explicit cancellation (IORING_OP_ASYNC_CANCEL) to safely terminate multishot
// operations before test cleanup.
//===----------------------------------------------------------------------===//
...
// connections. Each connection should produce a CQE with CQE_F_MORE set
// (except the last if we close the listener).
