...
TEST_P(LargeTransferTest, LargeTransfer_Chunked) {
  ...
  // Send 256KB total in 16KB chunks.
  ...
  std::vector<uint8_t> recv_buffer(kTotalSize);
  ...
  for (int chunk = 0; chunk < kNumChunks; ++chunk) {
    ...
    iree_host_size_t bytes_sent =
        SendAll(client, send_buffer.data() + chunk_offset, kChunkSize);
    ASSERT_EQ(bytes_sent, kChunkSize) << "Failed to send chunk " << chunk;
    total_sent += bytes_sent;
  }
  ASSERT_EQ(total_sent, kTotalSize);

  // Receive all data on server (may come in different chunk sizes).
  iree_host_size_t bytes_received =
      RecvAll(server, recv_buffer.data(), kTotalSize);
  ASSERT_EQ(bytes_received, kTotalSize) << "Failed to receive all data";
...
TEST_P(ConcurrentConnectionsTest, ConcurrentConnections_Parallel) {
  ...
  // Verify data integrity on each connection.
  for (int i = 0; i < kNumConnections; ++i) {
    ...
    EXPECT_EQ(memcmp(recv_buffers[i], send_data[i], expected_length), 0)
        << "Data mismatch on parallel connection " << i;
  }
