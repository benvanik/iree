...
TEST_P(ShutdownTest, Shutdown_InvalidMode) {
  ...
  // Invalid mode value.
  iree_status_t status =
      iree_async_socket_shutdown(client, (iree_async_socket_shutdown_mode_t)99);
  EXPECT_FALSE(iree_status_is_ok(status));
...
TEST_P(BindListenErrorTest, Bind_PortConflict) {
  ...
  // This should fail with FAILED_PRECONDITION (EADDRINUSE maps to this in IREE
  // because it represents a state precondition violation).
  iree_status_t status = iree_async_socket_bind(socket2, &bound_address);
  EXPECT_FALSE(iree_status_is_ok(status));
  EXPECT_EQ(iree_status_code(status), IREE_STATUS_FAILED_PRECONDITION);
  iree_status_ignore(status);
