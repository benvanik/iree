...
static LoopbackContext* CreateLoopbackContext(
  ...
  iree_async_address_from_ipv4(iree_string_view_empty(), 0, &bind_addr);
...
static void DestroyLoopbackContext(LoopbackContext* ctx) {
  if (!ctx) return;
  if (ctx->server) iree_async_socket_release(ctx->server);
  if (ctx->client) iree_async_socket_release(ctx->client);
  if (ctx->listener) iree_async_socket_release(ctx->listener);
  if (ctx->proactor) iree_async_proactor_release(ctx->proactor);
  delete ctx;
}
