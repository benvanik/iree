...
// Signal tests require POSIX signals (Linux, macOS, BSD).
// Windows uses console control handlers which have different semantics.
...
class SignalTest : public CtsTestBase<> {
 protected:
  ...
  // automatically by the proactor destroy path after signalfd teardown.
};
...
TEST_P(SignalTest, SignalDelivery) {
  ...
  // Poll to deliver the signal. Signal handling doesn't count as a user
  // completion (internal tag), so use PollOnce to process CQEs.
  PollOnce();
