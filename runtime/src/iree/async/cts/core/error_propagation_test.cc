...
TEST_P(ErrorPropagationTest, SendOnClosedConnectionSetsStickyFailure) {
  ...
  iree_status_t sticky_status = iree_async_socket_query_failure(client);
  EXPECT_FALSE(iree_status_is_ok(sticky_status))
      << "Socket should have sticky failure after I/O error";
  iree_status_ignore(sticky_status);
...
TEST_P(ErrorPropagationTest, ConnectFailurePropagatesThroughLinkedChain) {
  ...
  iree_status_t connect_status = connect_tracker.ConsumeStatus();
  EXPECT_FALSE(iree_status_is_ok(connect_status))
      << "Connect to non-listening port should fail";
