...
enum iree_async_socket_option_bits_e {
  ...
  // SO_ZEROCOPY: enables kernel zero-copy send path (MSG_ZEROCOPY on Linux).
  // All send operations on sockets with this option use the zero-copy path
  // (SEND_ZC on io_uring 6.0+), avoiding copies from user-space to kernel
  // buffers.
  //
  ...
  //
  // Requires the proactor to support
  // IREE_ASYNC_PROACTOR_CAPABILITY_ZERO_COPY_SEND. If the proactor doesn't
  // support it, create_socket returns IREE_STATUS_UNAVAILABLE.
...
// sets ZERO_COPY flag after setsockopt succeeds).
...
enum iree_async_socket_flag_bits_e {
  ...
  // Send operations use zero-copy path (SEND_ZC on io_uring 6.0+).
  // For create_socket: set when IREE_ASYNC_SOCKET_OPTION_ZERO_COPY succeeds.
  // For import_socket: caller must set this if they configured SO_ZEROCOPY.
...
//
// Capability checks:
//   If |options| requests a feature requiring a proactor capability that is not
//   present, creation fails immediately rather than silently degrading. For
//   example, requesting ZERO_COPY without
//   IREE_ASYNC_PROACTOR_CAPABILITY_ZERO_COPY_SEND returns
//   IREE_STATUS_UNAVAILABLE.
//
// Returns:
//   IREE_STATUS_OK: Socket created successfully.
//   IREE_STATUS_UNAVAILABLE: Requested option requires missing capability.
