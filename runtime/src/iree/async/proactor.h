...
// Forward declare notification flags (full definition in notification.h).
...
static inline void iree_async_proactor_release(
    iree_async_proactor_t* proactor) {
  if (iree_atomic_ref_count_dec(&proactor->ref_count) == 1) {
...
//   cleanup_fn, and the entry is freed via unregister or state_cleanup.
...
// On platforms without signal support (Windows), subscribe/unsubscribe are
// no-ops that return OK - use this to check if signal-based functionality
// will actually work before relying on it.
