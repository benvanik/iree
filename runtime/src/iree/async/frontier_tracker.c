...
//===----------------------------------------------------------------------===//
// Helper: Check if a waiter's frontier contains the given axis
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// Helper: Check if a waiter is satisfied or failed
//===----------------------------------------------------------------------===//
...
iree_status_t iree_async_frontier_tracker_initialize(
  ...
  if (axis_table_capacity > 0) {
    iree_host_size_t failure_array_size =
        axis_table_capacity * sizeof(iree_status_t);
    IREE_RETURN_IF_ERROR(
        iree_allocator_malloc(allocator, failure_array_size,
                              (void**)&tracker->axis_failure_statuses));
    memset(tracker->axis_failure_statuses, 0, failure_array_size);
  } else {
...
//===----------------------------------------------------------------------===//
// iree_async_frontier_tracker_deinitialize
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// iree_async_frontier_tracker_wait
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// iree_async_frontier_tracker_cancel_wait
//===----------------------------------------------------------------------===//
