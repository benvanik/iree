...
void TileDimOp::getAsmResultNames(
...
OpFoldResult TileDimOp::fold(FoldAdaptor adaptor) {
  auto tileType = cast<TileType>(getSource().getType());
  ...
  if (dimensionIndex < 0 || dimensionIndex >= tileType.getRank()) {
    return {};
  }

  // If dimension is static, fold to constant.
  int64_t dimSize = tileType.getDimSize(dimensionIndex);
...
// When the source tile is produced by an op that implements ShapeAwareOp,
// we can replace the dim query with the actual dynamic dimension value.
struct FoldTileDimThroughShapeAware : public OpRewritePattern<TileDimOp> {
  using OpRewritePattern<TileDimOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(TileDimOp op,
...
void TileDimOp::getCanonicalizationPatterns(RewritePatternSet& results,
                                            MLIRContext* context) {
  results.insert<FoldTileDimThroughShapeAware>(context);
}
