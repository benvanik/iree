...
FailureOr<bool> AssignRefOp::encode(SymbolTable &syms, VMFuncEncoder &e) {
  ...
  if (regAlloc) {
    int srcOrdinal = regAlloc->mapToRegister(getSource()).ordinal();
    int dstOrdinal = regAlloc->mapToRegister(getResult()).ordinal();
...
FailureOr<bool> CastRefAnyOp::encode(SymbolTable &syms, VMFuncEncoder &e) {
  ...
  if (regAlloc) {
    int srcOrdinal = regAlloc->mapToRegister(getOperand()).ordinal();
    int dstOrdinal = regAlloc->mapToRegister(getResult()).ordinal();
    if (srcOrdinal == dstOrdinal) {
      return false;
    }
