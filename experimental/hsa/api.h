...
typedef struct iree_hal_hsa_device_params_t {
  ...
  // // Normally we want to dispatch commands in command buffers in parallel,
  // given
  ...
  // // Normally we track resource lifetime in IREE explicitly, so we don't need
  // to
  // // incur Metal runtime overhead to do that. But good for debugging
  // purposes. iree_hal_metal_command_buffer_resource_reference_mode_t
  //     command_buffer_resource_reference_mode;

  // // Resource hazard tracking mode.
  // // IREE is following explicit GPU API model and tracks resource dependency
  // by
  ...
  // iree_hal_metal_resource_hazard_tracking_mode_t
  // resource_hazard_tracking_mode;
} iree_hal_hsa_device_params_t;

// Initializes |out_params| to default values.
void iree_hal_hsa_device_params_initialize(
...
// Creates an HSA HAL driver with the given |device_params|.
//
// |out_driver| must be released by the caller (see iree_hal_driver_release).
IREE_API_EXPORT iree_status_t iree_hal_hsa_driver_create_from_flags(
