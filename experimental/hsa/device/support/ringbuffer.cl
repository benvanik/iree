...
#define iree_amdgpu_ringbuffer_increment(value, capacity) \
  (((value) + 1u) % (capacity))
...
void iree_amdgpu_ringbuffer_uint64_enqueue(
  ...
  bool enqueued = false;
  do {
    enqueued = iree_amdgpu_ringbuffer_uint64_try_enqueue(ringbuffer, value);
    if (!enqueued) {
...
bool iree_amdgpu_ringbuffer_uint64_try_enqueue(
  ...
  uint32_t size = iree_amdgpu_atomic_load_explicit(
  ...
  while (!iree_amdgpu_atomic_compare_exchange_weak_explicit(
  ...
  while (true) {
    ...
    uint32_t write_index = iree_amdgpu_atomic_load_explicit(
        &ringbuffer->write_index, iree_amdgpu_memory_order_seq_cst,
        iree_amdgpu_memory_scope_device);
    while (!iree_amdgpu_atomic_compare_exchange_weak_explicit(
        &ringbuffer->write_index, &write_index,
        iree_amdgpu_ringbuffer_increment(write_index, ringbuffer->capacity),
    ...
    iree_amdgpu_ringbuffer_entry_state_t state =
        IREE_AMDGPU_RINGBUFFER_ENTRY_STATE_UNOCCUPIED;
    if (iree_amdgpu_atomic_compare_exchange_strong_explicit(
...
uint64_t iree_amdgpu_ringbuffer_uint64_dequeue(
  ...
  iree_amdgpu_ringbuffer_uint64_result_t result;
  do {
    result = iree_amdgpu_ringbuffer_uint64_try_dequeue(ringbuffer);
    if (!result.has_value) {
      // Yield for a few cycles - this isn't strictly required and may not even
      // help but makes me feel better.
      iree_amdgpu_yield(1);
...
iree_amdgpu_ringbuffer_uint64_try_dequeue(
    IREE_CL_GLOBAL iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT
        ringbuffer) {
  iree_amdgpu_ringbuffer_uint64_result_t result = {0, false};
  ...
  while (true) {
    ...
    uint32_t read_index = iree_amdgpu_atomic_load_explicit(
        &ringbuffer->read_index, iree_amdgpu_memory_order_seq_cst,
        iree_amdgpu_memory_scope_device);
    while (!iree_amdgpu_atomic_compare_exchange_weak_explicit(
        &ringbuffer->read_index, &read_index,
        iree_amdgpu_ringbuffer_increment(read_index, ringbuffer->capacity),
        iree_amdgpu_memory_order_seq_cst, iree_amdgpu_memory_order_seq_cst,
        iree_amdgpu_memory_scope_device));
