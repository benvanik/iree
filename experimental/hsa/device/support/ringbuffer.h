...
#ifndef IREE_HAL_DRIVERS_HSA_DEVICE_SUPPORT_RINGBUFFER_H_
#define IREE_HAL_DRIVERS_HSA_DEVICE_SUPPORT_RINGBUFFER_H_

#include "experimental/hsa/device/support/opencl.h"

//===----------------------------------------------------------------------===//
// iree_amdgpu_ringbuffer_uint64_t
//===----------------------------------------------------------------------===//

// Defines the state of an entry in the ringbuffer.
typedef uint32_t iree_amdgpu_ringbuffer_entry_state_t;
enum iree_amdgpu_ringbuffer_entry_state_e {
  // Entry is unoccupied (value is invalid).
  IREE_AMDGPU_RINGBUFFER_ENTRY_STATE_UNOCCUPIED = 0,
  // Entry is transitioning between unoccupied and occupied.
  IREE_AMDGPU_RINGBUFFER_ENTRY_STATE_TRANSITIONING,
  // Entry is occupied (value is valid).
  IREE_AMDGPU_RINGBUFFER_ENTRY_STATE_OCCUPIED,
};

// An entry in the ringbuffer representing a single value.
typedef struct IREE_CL_ALIGNAS(16) iree_amdgpu_ringbuffer_uint64_entry_t {
  // Current state of the entry used to block writers from conflicting.
  iree_amdgpu_atomic_uint32_t state;
  // Value stored in the entry.
  uint64_t value;
} iree_amdgpu_ringbuffer_uint64_entry_t;
...
typedef struct iree_amdgpu_ringbuffer_uint64_t {
  // Current number of elements in the ringbuffer.
  IREE_CL_ALIGNAS(iree_amdgpu_destructive_interference_size)
  iree_amdgpu_atomic_uint32_t size;
  ...
  // must be checked.
  IREE_CL_ALIGNAS(iree_amdgpu_destructive_interference_size)
  iree_amdgpu_atomic_uint32_t read_index;
  ...
  IREE_CL_ALIGNAS(iree_amdgpu_destructive_interference_size)
  iree_amdgpu_atomic_uint32_t write_index;
  // Zero-initialized entries containing the values of a slot in the ringbuffer
  // and a state indicating its validity.
  IREE_CL_ALIGNAS(iree_amdgpu_destructive_interference_size)
  iree_amdgpu_ringbuffer_uint64_entry_t entries[/*capacity*/];  // tail array
} iree_amdgpu_ringbuffer_uint64_t;

// Returns the total size in bytes required to store a ringbuffer with the
// specified capacity.
#define iree_amdgpu_ringbuffer_uint64_calculate_size(capacity) \
  sizeof(iree_amdgpu_ringbuffer_uint64_t) +                    \
      (capacity) * sizeof(iree_amdgpu_ringbuffer_uint64_entry_t)
...
static inline IREE_CL_ATTRIBUTE_ALWAYS_INLINE uint32_t
iree_amdgpu_ringbuffer_uint64_size(
    IREE_CL_GLOBAL const iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT
        ringbuffer) {
  return iree_amdgpu_atomic_load_explicit(&ringbuffer->size,
...
static inline IREE_CL_ATTRIBUTE_ALWAYS_INLINE uint32_t
iree_amdgpu_ringbuffer_uint64_capacity(
    IREE_CL_GLOBAL const iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT
        ringbuffer) {
...
static inline IREE_CL_ATTRIBUTE_ALWAYS_INLINE bool
iree_amdgpu_ringbuffer_uint64_empty(
    IREE_CL_GLOBAL const iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT
        ringbuffer) {
  return iree_amdgpu_ringbuffer_uint64_size(ringbuffer) == 0;
}

// Returns true if the ringbuffer is full.
static inline IREE_CL_ATTRIBUTE_ALWAYS_INLINE bool
iree_amdgpu_ringbuffer_uint64_full(
    IREE_CL_GLOBAL const iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT
        ringbuffer) {
  return iree_amdgpu_ringbuffer_uint64_size(ringbuffer) == ringbuffer->capacity;
}

// Enqueues a new value to the ringbuffer.
void iree_amdgpu_ringbuffer_uint64_enqueue(
    IREE_CL_GLOBAL iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT ringbuffer,
    uint64_t value);
...
// value is enqueued.
bool iree_amdgpu_ringbuffer_uint64_try_enqueue(
    IREE_CL_GLOBAL iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT ringbuffer,
    uint64_t value);

// Dequeues a value from the ringbuffer. Spins if the ringbuffer is empty.
uint64_t iree_amdgpu_ringbuffer_uint64_dequeue(
    IREE_CL_GLOBAL iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT
        ringbuffer);

typedef struct {
  uint64_t value;
  bool has_value;
} iree_amdgpu_ringbuffer_uint64_result_t;
...
static iree_amdgpu_ringbuffer_uint64_result_t
iree_amdgpu_ringbuffer_uint64_try_dequeue(
    IREE_CL_GLOBAL iree_amdgpu_ringbuffer_uint64_t* IREE_CL_RESTRICT
        ringbuffer);

#endif  // IREE_CL_TARGET_DEVICE

#endif  // IREE_HAL_DRIVERS_HSA_DEVICE_SUPPORT_RINGBUFFER_H_
