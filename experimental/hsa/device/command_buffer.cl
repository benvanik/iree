...
// iree_amdgpu_execution_state_enqueue_begin
...
static void iree_amdgpu_command_buffer_emplace_issue_block(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    const uint32_t queue_index) {
  // aql dispatch packet enqueue of `iree_amdgpu_command_buffer_issue_block`
  ...
  const iree_amdgpu_kernel_args_t control_args = state->kernels->issue_block;
  // DO NOT SUBMIT queue_index
  IREE_CL_GLOBAL hsa_kernel_dispatch_packet_t* control_packet = NULL;
...
void iree_amdgpu_command_buffer_enqueue(
  ...
  IREE_CL_GLOBAL const iree_amdgpu_command_block_t* block =
  ...
  iree_amdgpu_command_buffer_emplace_issue_block(state, block, queue_index);
...
//===----------------------------------------------------------------------===//
// IREE_AMDGPU_CMD_BARRIER
//===----------------------------------------------------------------------===//

static void iree_amdgpu_cmd_barrier_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_barrier_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  // DO NOT SUBMIT
...
static void iree_amdgpu_cmd_signal_event_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_signal_event_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
...
static void iree_amdgpu_cmd_reset_event_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_reset_event_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  // TODO(benvanik): HSA signal handling.
}
...
static void iree_amdgpu_cmd_wait_events_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_wait_events_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  // TODO(benvanik): HSA signal handling.
}
...
static void iree_amdgpu_cmd_fill_buffer_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_fill_buffer_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  IREE_CL_GLOBAL void* target_ptr =
      iree_amdgpu_buffer_ref_resolve(cmd->target_ref, state->bindings);
  const uint64_t length = cmd->target_ref.length;
  IREE_CL_GLOBAL uint64_t* kernargs_ptr =
      (IREE_CL_GLOBAL uint64_t*)(state->execution_kernarg_storage +
                                 cmd->kernarg_offset);
  iree_amdgpu_buffer_fill_emplace(target_ptr, length, cmd->pattern,
                                  cmd->pattern_length, state->kernels,
...
static void iree_amdgpu_cmd_copy_buffer_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_copy_buffer_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  IREE_CL_GLOBAL const void* source_ptr =
      iree_amdgpu_buffer_ref_resolve(cmd->source_ref, state->bindings);
  IREE_CL_GLOBAL void* target_ptr =
      iree_amdgpu_buffer_ref_resolve(cmd->target_ref, state->bindings);
  const uint64_t length = cmd->target_ref.length;
  IREE_CL_GLOBAL uint64_t* kernargs_ptr =
      (IREE_CL_GLOBAL uint64_t*)(state->execution_kernarg_storage +
                                 cmd->kernarg_offset);
  iree_amdgpu_buffer_copy_emplace(source_ptr, target_ptr, length,
                                  state->kernels,
                                  kernargs_ptr state->execution_queue,
                                  base_queue_index + cmd->header.packet_offset);
}
...
static IREE_CL_GLOBAL hsa_kernel_dispatch_packet_t*
iree_amdgpu_cmd_dispatch_prepare(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_dispatch_t* IREE_CL_RESTRICT cmd,
    IREE_CL_GLOBAL uint64_t* IREE_CL_RESTRICT kernarg_ptr,
    const uint32_t queue_index) {
  // DO NOT SUBMIT
  // verify this is correct
  for (uint16_t i = 0; i < cmd->binding_count; ++i) {
    ((IREE_CL_GLOBAL uint64_t*)kernarg_ptr)[i] =
        iree_amdgpu_buffer_ref_resolve(cmd->bindings[i], state->bindings);
  }
  ...
  const iree_amdgpu_kernel_args_t dispatch_args = cmd->packet.kernel_args;
  ...
  if (cmd->flags & IREE_AMDGPU_DISPATCH_FLAG_INDIRECT_STATIC) {
    ...
    IREE_CL_GLOBAL const uint32_t* workgroups_ptr =
        iree_amdgpu_buffer_ref_resolve(cmd->workgroups_ref, state->bindings);
  ...
  // is responsible for calling iree_amdgpu_cmd_dispatch_mark_ready when it is
  // ready for the hardware command processor to pick up the packet.
  return dispatch_packet;
}

static IREE_CL_ATTRIBUTE_ALWAYS_INLINE void iree_amdgpu_cmd_dispatch_commit(
    IREE_CL_GLOBAL const iree_amdgpu_cmd_dispatch_t* IREE_CL_RESTRICT cmd,
    IREE_CL_GLOBAL hsa_kernel_dispatch_packet_t* IREE_CL_RESTRICT
        dispatch_packet) {
  uint16_t header = HSA_PACKET_TYPE_KERNEL_DISPATCH << HSA_PACKET_HEADER_TYPE;
  if (cmd->header.flags & IREE_AMDGPU_CMD_FLAG_QUEUE_AWAIT_BARRIER) {
  ...
  // IREE_AMDGPU_CMD_FLAG_QUEUE_AWAIT_BARRIER?
  //
  // fence scopes?
...
static void iree_amdgpu_cmd_dispatch_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_dispatch_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  // Enqueue the dispatch packet but do not mark it as ready yet.
  IREE_CL_GLOBAL uint64_t* kernarg_ptr =
      (IREE_CL_GLOBAL uint64_t*)(state->execution_kernarg_storage +
                                 cmd->kernarg_offset);
  const uint32_t dispatch_index = base_queue_index + cmd->header.packet_offset;
  IREE_CL_GLOBAL hsa_kernel_dispatch_packet_t* dispatch_packet =
      iree_amdgpu_cmd_dispatch_prepare(state, block, cmd, kernarg_ptr,
                                       dispatch_index);

  // Mark the dispatch as complete and allow the hardware command processor to
  // process it.
  iree_amdgpu_cmd_dispatch_commit(cmd, dispatch_packet);
}
...
iree_amdgpu_command_buffer_workgroup_count_update(
    IREE_CL_GLOBAL const iree_amdgpu_cmd_dispatch_t* IREE_CL_RESTRICT cmd,
    IREE_CL_GLOBAL const uint32_t* IREE_CL_RESTRICT workgroups_ptr,
    IREE_CL_GLOBAL hsa_kernel_dispatch_packet_t* IREE_CL_RESTRICT
        dispatch_packet) {
...
static void iree_amdgpu_cmd_dispatch_indirect_dynamic_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_dispatch_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  ...
  // manipulate.
  IREE_CL_GLOBAL uint64_t* dispatch_kernarg_ptr =
      (IREE_CL_GLOBAL uint64_t*)(state->execution_kernarg_storage +
                                 cmd->kernarg_offset +
                                 IREE_AMDGPU_WORKGROUP_COUNT_UPDATE_KERNARG_SIZE);
  IREE_CL_GLOBAL hsa_kernel_dispatch_packet_t* dispatch_packet =
      iree_amdgpu_cmd_dispatch_prepare(state, block, cmd, dispatch_kernarg_ptr,
                                       dispatch_index);
  ...
  IREE_CL_GLOBAL void* update_kernargs[3] = {
      cmd,
      iree_amdgpu_buffer_ref_resolve(cmd->workgroups_ref, state->bindings),
      dispatch_packet,
  };
  // DO NOT SUBMIT implicit opencl args
  IREE_CL_GLOBAL void* update_kernarg_ptr =
  ...
  // Note that the header is not written until the end so that the
  // hardware command processor stalls until we're done writing.
  const iree_amdgpu_kernel_args_t update_args =
      state->kernels->workgroup_count_update;
  // DO NOT SUBMIT queue_index
  IREE_CL_GLOBAL hsa_kernel_dispatch_packet_t* update_packet = NULL;
...
static void iree_amdgpu_cmd_branch_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_branch_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  ...
  // populate the packet.
  //
  // NOTE: we implicitly assume IREE_AMDGPU_CMD_FLAG_QUEUE_AWAIT_BARRIER but
  // need not do so (technically) when continuing within the same command
  // buffer. Performing a barrier is a more conservative operation and may mask
  // compiler/command buffer construction issues with the more strict execution
  // model but in practice is unlikely to have an appreciable effect on latency.
  iree_amdgpu_command_buffer_emplace_issue_block(
...
static void iree_amdgpu_cmd_return_issue(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    IREE_CL_GLOBAL const iree_amdgpu_cmd_return_t* IREE_CL_RESTRICT cmd,
    const uint32_t base_queue_index) {
  ...
  // iree_amdgpu_barrier_and_emplace(deps, state->completion_signal)
  ...
  iree_amdgpu_queue_scheduler_enqueue(
      state->scheduler,
...
__kernel void iree_amdgpu_command_buffer_issue_block(
    IREE_CL_GLOBAL iree_amdgpu_execution_state_t* IREE_CL_RESTRICT state,
    IREE_CL_GLOBAL const iree_amdgpu_command_block_t* IREE_CL_RESTRICT block,
    const uint32_t base_queue_index) {
  // Each invocation handles a single command in the block.
  const uint32_t command_ordinal = iree_amdgpu_global_id_x();
  if (command_ordinal >= command_count) return;
  IREE_CL_GLOBAL const iree_amdgpu_cmd_t* cmd =
      &block->commands[command_ordinal];
  switch (cmd->header.type) {
    default:
      return;  // no-op
    case IREE_AMDGPU_CMD_BARRIER:
      return iree_amdgpu_cmd_barrier_issue(
          state, block,
          (IREE_CL_GLOBAL const iree_amdgpu_cmd_signal_event_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_SIGNAL_EVENT:
      return iree_amdgpu_cmd_signal_event_issue(
          state, block,
          (IREE_CL_GLOBAL const iree_amdgpu_cmd_signal_event_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_RESET_EVENT:
      return iree_amdgpu_cmd_reset_event_issue(
          state, block,
          (IREE_CL_GLOBAL const iree_amdgpu_cmd_reset_event_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_WAIT_EVENTS:
      return iree_amdgpu_cmd_wait_events_issue(
          state, block,
          (IREE_CL_GLOBAL const iree_amdgpu_cmd_wait_events_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_FILL_BUFFER:
      return iree_amdgpu_cmd_fill_buffer_issue(
          state, block,
          (IREE_CL_GLOBAL const iree_amdgpu_cmd_fill_buffer_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_COPY_BUFFER:
      return iree_amdgpu_cmd_copy_buffer_issue(
          state, block,
          (IREE_CL_GLOBAL const iree_amdgpu_cmd_copy_buffer_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_DISPATCH:
      return iree_amdgpu_cmd_dispatch_issue(
          state, block, (IREE_CL_GLOBAL const iree_amdgpu_cmd_dispatch_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_DISPATCH_INDIRECT_DYNAMIC:
      return iree_amdgpu_cmd_dispatch_indirect_dynamic_issue(
          state, block, (IREE_CL_GLOBAL const iree_amdgpu_cmd_dispatch_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_BRANCH:
      return iree_amdgpu_cmd_branch_issue(
          state, block, (IREE_CL_GLOBAL const iree_amdgpu_cmd_branch_t*)cmd,
          base_queue_index);
    case IREE_AMDGPU_CMD_RETURN:
      return iree_amdgpu_cmd_return_issue(
          state, block, (IREE_CL_GLOBAL const iree_amdgpu_cmd_return_t*)cmd,
          base_queue_index);
  }
}
